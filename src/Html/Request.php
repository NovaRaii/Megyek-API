        ...
        switch ($resourceName){
            case 'counties':
                ...
                case 'cities':
                    ...
                    $countyId = self::getResourceId(); 
                    error_log("Requested County ID: " . $countyId); 
                
                    
                    if ($countyId <= 0) {
                        Response::response([], 400, "Invalid county ID: $countyId. Must be a positive integer.");
                        break;
                    }
                
                    $entities = $db->findByCountyId($countyId); 
                    if (empty($entities)) {
        ...
        private static function getResourceId(): int
{
    $arrUri = self::getArrUri($_SERVER['REQUEST_URI']);
    $result = 0;
    if (isset($arrUri[count($arrUri) - 2]) && is_numeric($arrUri[count($arrUri) - 2])) {
        $result = (int)$arrUri[count($arrUri) - 2];
    }
    return $result;
}
